...
system_prompt = f"""
    ...
    질문에 대답하기 전에, 반드시 다음 6단계 과정 및 추가사항을 거쳐서 사고하세요:
    ...
    6단계:답변이 두개일 시 틀니 -원, 임플란트 -원 같이 간결하게 작성한다.
    
    추가사항:적용되지 않을때는 "해당되지 않음" 으로 답변한다.
...
for item, answer in zip(datasets, answer_key_list):
    ...
    model_answer = self_consistency(question, n=5, temperature=0.5)
